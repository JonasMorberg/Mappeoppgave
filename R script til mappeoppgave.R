...
df <- subset(df_growth2019, select = c("ln_gdppc", "ln_gdppc0", "avg_gdpgrowth", "avg_p", "avg_nsy", "avg_educ", "avg_nry", "avg_gi", "avg_n", "avg_gx"))
# Gi beskrivende navn til variablene (i samme rekkefølge som de ligger i datasettet)
labs <- c("log BNP per innbygger(2019)", "log BNP per innbygger(2000)", "Gjennomsnitlig årlig vekstrate i BNP pc 2000-2019 (%)", "Gjennomsnittlig årlig befolkningsvekst (%)", "Gjennomsnittlig netto-sparing(%)", "Gjennomsnittlig humankapital(%)", "Gjennomsnittlig uttømning av naturressurser(%)", "Gjennomsnittlig årlig vekst i investeringer(%)", "Gjennomsnittlig årlig vekstrate i arbeidskraften(%)", "Gjennomsnittlig vekstrate i eksport(%)") 
...
plot1 <- ggplot(df_growth2019_n, aes(x = avg_nsy , y = ln_gdppc, na.rm = TRUE)) +
  xlab("Gjennomsnittlig sparing for 2000-2015") + # Beskrivelse for x-akselen
  ylab("log BNP per innbygger 2019") + # Beskrivelse for y-akselen
  labs(title = "Sammenhengen mellom Sparing og BNP per innbygger") +
  ...
  scale_y_continuous(trans = 'log2', breaks=c(7, 8, 9, 10, 11)) + # logaritmere BNP pc og velg hvilke "ticks" som skal vises
  scale_x_continuous(breaks=c(-5, 0, 5, 10, 15, 20, 25)) # Velg vilke "ticks" som skal vises på x-akselen
plot1


plot5 <- ggplot(df_growth2019_n, aes(x = avg_p , y = ln_gdppc, na.rm = TRUE)) +
  xlab("Gjennomsnittlig årlig befolkningsvekst") + # Beskrivelse for x-akselen
  ylab("log BNP per innbygger 2019") + # Beskrivelse for y-akselen
  labs(title = "Sammenhengen mellom befolkningsvekst og BNP per innbygger") +
  theme_minimal(base_size = 14, base_family = "Georgia") + # Tekststørrelse og font
  geom_point(aes(size = poptot, color = region), alpha = 0.6) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_x_continuous(labels = dollar)  + # Legg til et dollar tegn til y-akselen
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
  scale_y_continuous(trans = 'log2', breaks=c(7, 8, 9, 10, 11)) + # logaritmere BNP pc og velg hvilke "ticks" som skal vises
  scale_x_continuous(breaks=c(-1, 0, 1, 2, 3, 4)) # Velg vilke "ticks" som skal vises på x-akselen
plot5

plot6 <- ggplot(df_growth2019_n, aes(x = avg_educ , y = ln_gdppc, na.rm = TRUE)) +
  xlab("Gjennomsnittlig antall år i skole") + # Beskrivelse for x-akselen
  ylab("log BNP per innbygger 2019") + # Beskrivelse for y-akselen
  labs(title = "Sammenhengen mellom humankapital og BNP per innbygger") +
  theme_minimal(base_size = 14, base_family = "Georgia") + # Tekststørrelse og font
  geom_point(aes(size = poptot, color = region), alpha = 0.6) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_x_continuous(labels = dollar)  + # Legg til et dollar tegn til y-akselen
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
  scale_y_continuous(trans = 'log2', breaks=c(7, 8, 9, 10, 11)) + # logaritmere BNP pc og velg hvilke "ticks" som skal vises
  scale_x_continuous(breaks=c(0, 2, 4, 6, 8, 10, 12)) # Velg vilke "ticks" som skal vises på x-akselen
plot6

plot2 <- ggplot(df_growth2019_n, aes(x = avg_nsy , y = avg_gdpgrowth, na.rm = TRUE)) +
  xlab("Gjennomsnittlig sparing for 2000-2015") + # Beskrivelse for x-akselen
  ylab("Årlig vekstrate i BNP per innbygger") + # Beskrivelse for y-akselen
  labs(title = "Sammenhengen mellom Sparing og vekst i BNP per innbygger") +
  theme_minimal(base_size = 14, base_family = "Georgia") + # Tekststørrelse og font
  geom_point(aes(size = poptot, color = region), alpha = 0.6) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_x_continuous(labels = dollar)  + # Legg til et dollar tegn til y-akselen
  scale_size_area(guide = "none", max_size = 15) + #Ta vekk legend for befolkningsstørrelse
  theme(legend.text = element_text(size = 10,color="black"))+  # Bestem font-størrelse på legend
  scale_colour_manual(values = rainbow(9)) +# Velg farger til bobblene
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
  scale_y_continuous(trans = 'log2', breaks=c(0.25, 0.5, 1, 2, 4, 6, 8)) + # logaritmere BNP pc og velg hvilke "ticks" som skal vises
  scale_x_continuous(breaks=c(-5, 0, 5, 10, 15, 20, 25)) # Velg vilke "ticks" som skal vises på x-akselen
plot2

plot3 <- ggplot(df_growth2019_n, aes(x = avg_educ , y = avg_gdpgrowth, na.rm = TRUE)) +
  xlab("Gjennomsnittlig antall år i skole") + # Beskrivelse for x-akselen
  ylab("Årlig vekstrate i BNP per innbygger") + # Beskrivelse for y-akselen
  labs(title = "Sammenhengen mellom Humankapital og vekst i BNP per innbygger") +
  theme_minimal(base_size = 14, base_family = "Georgia") + # Tekststørrelse og font
  geom_point(aes(size = poptot, color = region), alpha = 0.6) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_x_continuous(labels = dollar)  + # Legg til et dollar tegn til y-akselen
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
  scale_y_continuous(trans = 'log2', breaks=c(0.25, 0.5, 1, 2, 4, 6, 8)) + # logaritmere BNP pc og velg hvilke "ticks" som skal vises
  scale_x_continuous(breaks=c(-8 ,-5, -2, 1, 4, 7, 10, 13, 16, 19, 22, 25, 28)) # Velg vilke "ticks" som skal vises på x-akselen
plot3
...
model1 <- lm(avg_gdpgrowth  ~ avg_nsy + avg_p + avg_educ + avg_gi + avg_nry + ln_gdppc + avg_n, data= df_growth2019)
...
model2 <- lm(avg_gdpgrowth  ~ avg_nsy + avg_p + avg_educ + avg_gi + avg_nry + ln_gdppc + avg_n, data= no_outliers)

tab_model(model2)

model3 <- lm(avg_gdpgrowth  ~ avg_nsy + avg_p + avg_educ + avg_gi + avg_nry + ln_gdppc + avg_n + avg_gx, data= no_outliers)
